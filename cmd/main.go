...
func main() {
	...
	if err := (&controller.LabReconciler{
	...
	if err := (&controller.KNLConfigReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "KNLConfig")
		os.Exit(1)
	}
	// nolint:goconst
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1beta1.SetupKNLConfigWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "KNLConfig")
			os.Exit(1)
		}
	}
